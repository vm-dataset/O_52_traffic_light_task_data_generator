...
class VideoGenerator:
    ...
    def __init__(self, fps: int = 10, output_format: str = "mp4"):
        ...
        # Use H.264 for mp4 (better QuickTime compatibility) or XVID for avi
        if output_format == "mp4":
            # Try H.264 codecs in order of preference
            # 'avc1' is the most compatible with QuickTime/macOS
            self.codec = 'avc1'  # H.264 codec for maximum compatibility
            self.extension = '.mp4'
        else:
            self.codec = 'XVID'
            self.extension = '.avi'
    ...
    def _ensure_yuv420p(self, video_path: Path) -> None:
        """
        Ensure video uses yuv420p pixel format for maximum compatibility.
        
        Uses ffmpeg to re-encode if available. This is critical for QuickTime
        and other players that don't support all pixel formats.
        
        Args:
            video_path: Path to video file to check/convert
        """
        import subprocess
        import shutil
        ...
        if not shutil.which('ffmpeg'):
            print("⚠️  Warning: ffmpeg not found. Video may not be compatible with QuickTime.")
            print("   Install ffmpeg: brew install ffmpeg (macOS) or apt-get install ffmpeg (Linux)")
            return
        
        try:
            # Check current pixel format
            probe_cmd = [
                'ffprobe',
                '-v', 'error',
                '-select_streams', 'v:0',
            ...
            current_pix_fmt = result.stdout.strip()
            
            # If already yuv420p, no need to re-encode
            if current_pix_fmt == 'yuv420p':
                return
            
            # Re-encode to yuv420p using H.264
            temp_path = video_path.with_suffix('.tmp.mp4')
            convert_cmd = [
                'ffmpeg',
                '-i', str(video_path),
                '-c:v', 'libx264',        # Use H.264 codec
                '-pix_fmt', 'yuv420p',    # Force yuv420p pixel format
            ...
            print(f"⚠️  Warning: ffmpeg conversion timed out for {video_path}")
        except subprocess.CalledProcessError as e:
            print(f"⚠️  Warning: ffmpeg conversion failed for {video_path}")
            print(f"   Error: {e.stderr.decode() if e.stderr else 'Unknown error'}")
        except Exception as e:
            print(f"⚠️  Warning: Failed to ensure yuv420p format: {e}")
    ...
    def create_video_from_frames(
        ...
        # Create video writer
        # Note: isColor=True ensures proper color encoding
        ...
        if not writer.isOpened():
            ...
            print(f"Warning: Failed to open video writer with codec '{self.codec}', trying 'mp4v'")
        ...
        # Post-process video to ensure yuv420p pixel format for maximum compatibility
        # This is critical for QuickTime/macOS compatibility
        self._ensure_yuv420p(output_path)
